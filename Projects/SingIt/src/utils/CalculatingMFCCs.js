import { extract, compare } from "meyda";

// 첫 번째 오디오 파일을 로드하고 MFCC를 추출합니다.
async function MFCC() {
  // 첫 번째 오디오 파일을 로드하고 MFCC를 추출합니다.
  const response1 = await fetch("audio1.mp3");
  const arrayBuffer1 = await response1.arrayBuffer();
  const audioBuffer1 = await new AudioContext().decodeAudioData(arrayBuffer1);
  const mfcc1 = extract("mfcc", audioBuffer1.getChannelData(0));

  // 두 번째 오디오 파일을 로드하고 MFCC를 추출합니다.
  const response2 = await fetch("audio2.mp3");
  const arrayBuffer2 = await response2.arrayBuffer();
  const audioBuffer2 = await new AudioContext().decodeAudioData(arrayBuffer2);
  const mfcc2 = extract("mfcc", audioBuffer2.getChannelData(0));

  // 두 MFCC 배열 간의 유사도를 비교합니다.
  const similarity = compare(mfcc1, mfcc2);
